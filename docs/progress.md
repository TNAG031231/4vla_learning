...
## Phase 0.4a-4 Temporal + Waypoint Dataset

- 2026-09-13 在本机 `codex4vla_env` 完成真实 train/validation intake、生成和落盘回读验证；输入为下载自 AutoDL 的 frozen source projection 与 factorized targets，两者未重新派生或覆盖。
- Schema `phase0.4-temporal-waypoint-v0.1`；冻结 `T_hist=3`（当前加两帧 past keyframe），顺序 oldest → current，shortage policy 为左侧 `null_padding`。Padding 各字段为 null、mask=false；真实观测 mask=true，ego-motion availability 单独保留。
- 候选长度 1/2/3/4 的完整历史数：train 为 `14253/13868/13476/13085`，validation 为 `3594/3500/3401/3309`。长度 3 覆盖率为 `94.5485%/94.6299%`，提供短时上下文且每条最多三张图；多图显存成本尚未实测。
- Final counts：train `14253` / `560 scenes`，validation `3594` / `140 scenes`；full/padded/unavailable 分别为 `13476/777/0` 与 `3401/193/0`。实际 1/2/3 帧分布分别为 `385/392/13476` 与 `94/99/3401`，全部样本保留。
