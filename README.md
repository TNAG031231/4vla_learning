...
已执行的新 pilot 扫描 804 samples，纳入 `phase0_trainval_dataset_manifest_v1` 533 条，排除 271 条：`insufficient_remaining_horizon=124`、`timestamp_out_of_tolerance=147`，其余排除原因均为 0。motion availability 为 `full=501`、`partial=17`、`unavailable=15`；路径泄漏与 scene split overlap 检查均通过。完整 850-scene mapping 已冻结到 Git 外 sidecar，mapping SHA-256 为 `a96e04aaf068e75b0aa3ecb8412dc5b35fea2412d7090bbee0a6661132923b12`，scene histogram SHA-256 为 `0cee51a6f64e3f2e10382ca7672cc0aa1386065a3fe8a1f927f5469e211a11a2`。完整扫描确认 108 个已有 audit token 全部匹配、0 过滤、0 缺失；本次固定 20-scene pilot 未抽中这些 token，因此记录为 `audited=0`、`unaudited=533`。完整 850-scene manifest 构建与分层人工抽检仍为下一 gate。
